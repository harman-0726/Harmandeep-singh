...
import { Camera, Cpu, Sparkles, RefreshCw } from "lucide-react";
...
export default function VisionCanvas() {
  ...
  useEffect(() => {
    // Generate computer vision joint marker coordinate nodes simulating facial skeletal mesh
    const labels = ["Forehead", "L-Eye", "R-Eye", "Nose", "L-Cheek", "R-Cheek", "Mouth", "L-Jaw", "R-Jaw", "Chin"];
    const basePoints = [
      { x: 200, y: 70, label: "FX-01" }, // Forehead
      { x: 160, y: 110, label: "LE-02" }, // L-Eye
      { x: 240, y: 110, label: "RE-03" }, // R-Eye
      { x: 200, y: 140, label: "NS-04" }, // Nose
      { x: 140, y: 160, label: "LC-05" }, // L-Cheek
      { x: 260, y: 160, label: "RC-06" }, // R-Cheek
      { x: 200, y: 195, label: "MT-07" }, // Mouth
      { x: 120, y: 220, label: "LJ-08" }, // L-Jaw
      { x: 280, y: 220, label: "RJ-09" }, // R-Jaw
      { x: 200, y: 255, label: "CH-10" }, // Chin
    ];
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Scale calculations for responsive centering
      ...
      // Update and draw skeletal nodes
      const points = pointsRef.current;
      points.forEach((p) => {
        // Targeted interactive magnetic coordinates pulls towards cursor
        ...
        if (dist < 150) {
          ...
          // Pulling force factor
          fx = (dx / dist) * force * 15;
          fy = (dy / dist) * force * 15;
        }

        // Spring restoration physics back to origin
        ...
        if (activeTab === "skeleton") {
          ...
          // Outer pulsing coordinate ring on nearest items
          if (dist < 100) {
            ctx.beginPath();
            ...
            // Coordinate label telemetry readout
      ...
      // Joint connection paths (Triangulation matrices representation)
      if (activeTab === "skeleton" && points.length >= 10) {
        ...
        // Outer facial jaw border
        connect(0, 1); connect(0, 2); // Forehead to Eyes
        connect(1, 4); connect(2, 6); // Eyes to Cheeks
        connect(4, 7); connect(6, 9); // Cheeks to Jaws
        connect(7, 9); // Jaws connection
        
        // Semantic cluster center linkages
        connect(1, 3); connect(2, 3); // Eyes to Nose
        connect(3, 4); connect(3, 6); // Nose to Cheeks
        connect(3, 5); // Nose to Mouth
        connect(5, 7); connect(5, 9); // Mouth to Jaw
        connect(5, 8); // Mouth to Jaw
        connect(8, 9); // Chin Link
      }

      // Render interactive coordinate scan crosshair
      if (isHovering) {
        ctx.lineWidth = 0.5;
  ...
  return (
    <div ref={containerRef} className="relative w-full rounded-2xl bg-[#0F0F0F] border border-white/10 p-4 shadow-2xl overflow-hidden aspect-[4/3] flex flex-col group justify-between">
