...
import { MessageSquare, X, Send, Sparkles, AlertCircle, Bot, CornerDownLeft, Minimize2, ArrowRight } from "lucide-react";
...
export default function AiTwin({ isEmbedded = false }: { isEmbedded?: boolean }) {
  ...
  const handleSend = async (textToSend: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Local gateway offline or key missing.");
      }
      ...
      console.error(err);
      setErrorStatus("Twin stream error. Falling back to offline responses.");
      // Fallback response
      setTimeout(() => {
        setMessages((prev) => [
          ...prev,
          {
            role: "model",
            text: `Hi from offline simulator mode. Harmandeep specializes in advanced Python scripting, machine learning modeling, and high-performance backend systems. Drop your message in the contact form or email directly: 01harmandeep@gmail.com!`,
          },
        ]);
      }, 600);
    } finally {
  ...
  // Embedded rendering (used straight in the project grid view)
  if (isEmbedded) {
    return (
        ...
        <div className="flex-1 p-4 overflow-y-auto space-y-4 bg-[#0D0D0D] scrollbar-thin">
          {messages.map((msg, i) => (
            ...
            >
              ...
              <div
                className={`px-3.5 py-2.5 rounded-lg text-sm transition-all duration-200 break-words leading-relaxed ${
                  msg.role === "user"
  ...
  return (
    <>
      ...
      {/* Floating Chat Container with Glassmorphism */}
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-50 w-[92vw] sm:w-[420px] h-[500px] rounded-2xl flex flex-col shadow-[0_12px_44px_rgba(0,0,0,0.4)] glass-panel text-white animate-fade-in-up border border-white/10 overflow-hidden bg-[#0F0F0FCC]">
              ...
              >
                <div
                  className={`text-[9px] font-mono mb-0.5 ${
                    msg.role === "user" ? "text-brand-orange/90" : "text-zinc-500"
                  }`}
                >
